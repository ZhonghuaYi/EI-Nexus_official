...
def gt_matches_from_pose_depth(
    ...
    **kw
    ...
    """
    Compute ground truth matches from pose and depth maps.
    
    Args:
        kp0: (B, N, 2) tensor of keypoints in view 0
        kp1: (B, M, 2) tensor of keypoints in view 1
        camera0: Camera object for view 0
        camera1: Camera object for view 1
        depth0: (B, 1, H, W) tensor of depth maps in view 0
        depth1: (B, 1, H, W) tensor of depth maps in view 1
        T_0to1: (B, 4, 4) tensor of poses from view 0 to view 1
        T_1to0: (B, 4, 4) tensor of poses from view 1 to view 0
        pos_th: positive threshold in pixels
        neg_th: negative threshold in pixels
        epi_th: epipolar threshold in pixels
        cc_th: circle consistency threshold in pixels
        kw: additional keyword arguments, "depth_keypoints0", "depth_keypoints1", "valid_depth_keypoints0", "valid_depth_keypoints1"
        
    Returns:
        a dict containing:
            - assignment: (B, N, M) tensor of positive matches
            - reward: (B, N, M) tensor of matching rewards
            ...
            - depth_keypoints0: (B, N) tensor of depths of keypoints in view 0
            - depth_keypoints1: (B, M) tensor of depths of keypoints in view 1
            - proj_0to1: (B, N, 2) tensor of keypoints in view 1
            - proj_1to0: (B, M, 2) tensor of keypoints in view 0
            - visible0: (B, N) tensor of visibility of keypoints in view 0
            - visible1: (B, M) tensor of visibility of keypoints in view 1
    """
    ...
    # print(f'reward: {positive.shape}')
    # print(f'matches0: {m0.shape}')
    # print(f'matches1: {m1.shape}')
    # print(f'matching_scores0: {(m0 > -1).float().shape}')
    # print(f'matching_scores1: {(m1 > -1).float().shape}')
    # print(f'depth_keypoints0: {d0.shape}')
    # print(f'depth_keypoints1: {d1.shape}')
    # print(f'proj_0to1: {kp0_1.shape}')
    # print(f'proj_1to0: {kp1_0.shape}')
    # print(f'visible0: {visible0.shape}')
    # print(f'visible1: {visible1.shape}')
    return {
        "assignment": positive,
