...
class LightGlue(nn.Module):
    default_conf = {
        ...
        "checkpointed": False,
        "weights": "superpoint",  # either a path or the name of pretrained weights (disk, ...)
    ...
    def forward(self, feats0: torch.Tensor, feats1: torch.Tensor) -> Dict[str, torch.Tensor]:
        ...
        origin_kpts0 = feats0['sparse_positions'][..., :2]
        origin_kpts1 = feats1['sparse_positions'][..., :2]
        b, m, _ = origin_kpts0.shape
        b, n, _ = origin_kpts1.shape
        device = origin_kpts0.device
        # if "view0" in data.keys() and "view1" in data.keys():
        #     size0 = data["view0"].get("image_size")
        #     size1 = data["view1"].get("image_size")
        size0 = feats0['image_size'][0]
        size1 = feats1['image_size'][0]
        kpts0 = normalize_keypoints(origin_kpts0, size0).clone()
        kpts1 = normalize_keypoints(origin_kpts1, size1).clone()
        ...
        # encoding0 = torch.zeros_like(encoding0, device=device)
        # encoding1 = torch.zeros_like(encoding1, device=device)
        ...
        matches0, matches1, mscores0, mscores1 = filter_matches(scores, self.conf.filter_threshold)
        ...
        if b > 1:
            ...
            for j in range(m):
                if matches0[0][j] > -1:
                    matched_kpts0.append(origin_kpts0[0][j])
                    matched_kpts1.append(origin_kpts1[0][matches0[0][j]])
