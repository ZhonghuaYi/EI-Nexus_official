...
from .homography import HomographicSampler
...
def prob_map_to_points_map(
    ...
    if top_k:
        if top_k >= prob_map.shape[-1] * prob_map.shape[-2]:
            ...
            top_k_percentile = (
                reshaped_prob_map[0].size()[0] - top_k
...
def prob_map_to_positions_with_prob(
    ...
    if ordering == "xy":
        positions = tuple(p.flip(-1) for p in positions)
    prob = tuple(
...
def get_dense_meshgrid(h, w, device, batch_size=None):
    dense_meshgrid = HomographicSampler._create_meshgrid(
        w,
        h,
        device=device,
        normalized=False,
    )
    dense_meshgrid = dense_meshgrid.permute(0, 2, 1, 3)
    dense_meshgrid = dense_meshgrid.reshape(-1, 2)
    dense_meshgrid = dense_meshgrid.unsqueeze(0)

    if batch_size is not None:
        dense_meshgrid = dense_meshgrid.expand(batch_size, -1, -1)

    return dense_meshgrid


def get_dense_positions(probability, ordering="yx"):
    batch_size = probability.shape[0]
    device = probability.device
    dense_positions = get_dense_meshgrid(
        probability.shape[2],
        probability.shape[3],
        device,
    ...
    if ordering == "xy":
        dense_positions = dense_positions.flip(-1)
